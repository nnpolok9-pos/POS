...
const SectionHeader = ({ title, accent = "bg-[#ef3d2f]" }) => (
  <div className="mb-[0.55vw] flex items-center gap-[0.4vw]">
    <div className={`h-[0.85vw] w-[3vw] rounded-[0.2vw] ${accent}`} />
    <h2 className="font-display text-[1.65vw] font-black uppercase leading-none tracking-tight text-[#1d1d1d]">
      {title}
    </h2>
  </div>
);
...
const ProductTile = ({ product, indexLabel }) => (
  <div className="rounded-[1vw] bg-white/65 p-[0.55vw] shadow-[0_8px_22px_rgba(15,23,42,0.08)]">
    <div className="flex items-start justify-between gap-[0.5vw]">
      <div className="min-w-0">
        ...
        <p className="mt-[0.12vw] line-clamp-2 text-[0.8vw] font-extrabold leading-[1.05] text-[#151515]">
          {product.name}
        </p>
      </div>
      <div className="h-[4.4vw] w-[4.4vw] shrink-0 overflow-hidden rounded-[0.9vw] bg-white">
...
const GridSection = ({ title, products }) => (
  <section className="rounded-[1.5vw] bg-[linear-gradient(180deg,rgba(255,255,255,0.96)_0%,rgba(247,245,252,0.9)_100%)] p-[0.9vw] shadow-[0_16px_32px_rgba(15,23,42,0.12)]">
...
const FeatureSection = ({ title, products }) => {
  ...
  return (
    <section className="rounded-[1.5vw] bg-[linear-gradient(180deg,rgba(255,255,255,0.97)_0%,rgba(246,244,251,0.92)_100%)] p-[0.95vw] shadow-[0_16px_32px_rgba(15,23,42,0.12)]">
      <SectionHeader title={title} />
      <div className="grid h-full grid-cols-[minmax(0,1fr)_42%] gap-[0.8vw]">
        <div className="space-y-[0.55vw]">
          {list.length > 0 ? (
            list.map((product, index) => (
              <div key={product.id} className="rounded-[1vw] bg-white/72 p-[0.55vw] shadow-[0_8px_20px_rgba(15,23,42,0.08)]">
                <div className="flex items-start justify-between gap-[0.6vw]">
                  <div className="min-w-0">
                    ...
                    <p className="mt-[0.12vw] line-clamp-2 text-[0.84vw] font-black leading-[1.05] text-[#171717]">{product.name}</p>
                    {product.description ? (
                      <p className="mt-[0.18vw] line-clamp-2 text-[0.58vw] leading-[1.25] text-slate-500">{product.description}</p>
                    ) : null}
                  </div>
                  <ProductPrice product={product} />
                </div>
        ...
        <div className="relative overflow-hidden rounded-[1.25vw] bg-[radial-gradient(circle_at_top,rgba(255,255,255,0.92),rgba(232,230,239,0.95))]">
    ...
    <div className="grid grid-cols-2 gap-[0.55vw]">
      {products.slice(0, 6).map((product) => (
        <div key={product.id} className="flex items-center gap-[0.5vw] rounded-[0.95vw] bg-white/72 p-[0.45vw] shadow-[0_8px_18px_rgba(15,23,42,0.07)]">
          ...
          <div className="min-w-0">
            <p className="line-clamp-2 text-[0.74vw] font-extrabold leading-[1.05] text-[#171717]">{product.name}</p>
...
const DrinksStrip = ({ products }) => (
  <section className="rounded-[1.5vw] bg-[linear-gradient(180deg,rgba(255,255,255,0.97)_0%,rgba(245,243,250,0.94)_100%)] p-[0.85vw] shadow-[0_16px_32px_rgba(15,23,42,0.12)]">
...
const FooterBar = ({ shopName }) => (
  <footer className="grid shrink-0 grid-cols-[auto_1fr_auto] items-center gap-[1vw] rounded-[1vw] bg-[#ef3d2f] px-[1vw] py-[0.55vw] text-white shadow-[0_14px_24px_rgba(239,61,47,0.3)]">
    <div className="text-[0.86vw] font-black uppercase tracking-[0.08em]">{shopName}</div>
    <div className="text-center text-[0.68vw] font-semibold uppercase tracking-[0.18em] text-white/90">
      Fresh food menu display
    </div>
    <div className="text-[0.72vw] font-black uppercase tracking-[0.08em]">ASENPOS</div>
  </footer>
);
...
const MenuCardPosterPage = () => {
  ...
  return (
    <div className="h-screen overflow-hidden bg-[radial-gradient(circle_at_top,_rgba(255,255,255,0.95),_rgba(232,230,242,0.98))] p-[0.7vw] text-slate-900">
      <div className="mx-auto flex h-full max-w-[1920px] flex-col gap-[0.65vw]">
        <header className="grid shrink-0 grid-cols-[auto_1fr_auto] items-center gap-[0.85vw] rounded-[1.5vw] bg-[linear-gradient(180deg,rgba(255,255,255,0.98)_0%,rgba(244,242,250,0.95)_100%)] px-[1vw] py-[0.75vw] shadow-[0_18px_36px_rgba(15,23,42,0.12)]">
          ...
          <div className="min-w-0">
            <p className="text-[0.62vw] font-black uppercase tracking-[0.26em] text-[#ef3d2f]">Display Menu</p>
        ...
        {loading ? (
          ...
          <>
            <main className="grid min-h-0 flex-1 grid-cols-12 gap-[0.7vw]">
              ...
              <div className="col-span-8 grid min-h-0 grid-rows-[1.1fr_0.9fr] gap-[0.7vw]">
                {feature ? <FeatureSection title={feature.category} products={feature.products} /> : <div className="rounded-[1.5vw] bg-white/75" />}

                <div className="grid min-h-0 grid-cols-2 gap-[0.7vw]">
                  ...
                  {extraSections[0] ? (
                    ...
                    <CompactSection title={`${leftTop.category} Specials`} products={leftTop.products.slice(0, 6)} />
                  ) : (
            ...
            <div className="grid shrink-0 grid-cols-[1.15fr_1fr] gap-[0.7vw]">
              {drinksSection ? (
              ...
              <FooterBar shopName={shop.shopName} />
            </div>
          </>
        )}
      </div>
