...
const POSTER_TITLE = "ASEN MENU SHOWCASE KH";
const HALAL_LOGO_PATH = "/halal-logo-final.png";

const getKhmerName = (product) => product.khmerName || product.name || "";
const getKhmerDescription = (product) => product.khmerDescription || product.description || "";
const getKhmerCategory = (product) => product.khmerCategory || product.category || "Menu";
      ...
      new Map(
        products.map((product) => [
          product.category || "Menu",
          {
            ...
            title: getKhmerCategory(product)
          }
        ])
      ).values()
    )
...
const ComboImageCard = ({ product, soft = false, flat = false }) => (
  ...
  >
    <div className="overflow-hidden bg-[radial-gradient(circle_at_top,rgba(255,255,255,0.96),rgba(236,234,244,0.94))]">
      <img src={imageUrl(product.image)} alt={getKhmerName(product)} className="h-full w-full object-contain p-[0.15vw]" />
    </div>
    <div className="flex flex-col justify-between p-[0.36vw]">
      <div>
        <p className="line-clamp-2 text-[0.98vw] font-black leading-[1.04] text-[#171717]">{getKhmerName(product)}</p>
        {getKhmerDescription(product) ? (
          <p className="mt-[0.12vw] line-clamp-3 text-[0.48vw] leading-[1.14] text-slate-500">{getKhmerDescription(product)}</p>
        ) : null}
      </div>
  ...
  <div className={`h-full rounded-[0.9vw] bg-white/78 ${largeImage ? "p-[0.18vw]" : "p-[0.22vw]"} shadow-[0_8px_22px_rgba(15,23,42,0.08)]`}>
    <div className={`grid h-full items-start gap-[0.28vw] ${largeImage ? "grid-cols-[minmax(0,1fr)_7.2vw]" : "grid-cols-[minmax(0,1fr)_6vw]"}`}>
      <div className="min-w-0 self-start pt-[0.02vw]">
        ...
        {showDescription && getKhmerDescription(product) ? (
      ...
      <div className={`${largeImage ? "h-[4.8vw] w-[7.2vw]" : "h-[6vw] w-[6vw]"} shrink-0 self-center overflow-hidden rounded-[0.8vw] bg-white`}>
        <img src={imageUrl(product.image)} alt={getKhmerName(product)} className={`h-full w-full object-contain ${largeImage ? "p-[0.02vw]" : "p-0"}`} />
      </div>
    </div>
  </div>
);
    ...
    <section className="flex h-full flex-col overflow-hidden rounded-[1.32vw] bg-[linear-gradient(180deg,rgba(255,255,255,0.97)_0%,rgba(246,244,251,0.94)_100%)] px-[0.52vw] pb-[0.52vw] pt-[0.02vw] shadow-[0_14px_28px_rgba(15,23,42,0.10)]">
      ...
      <div className="grid flex-1 grid-cols-[minmax(0,1fr)_44%] gap-[0.42vw]">
              ...
              <div key={product.id} className="rounded-[0.92vw] bg-white/82 p-[0.32vw] shadow-[0_8px_20px_rgba(15,23,42,0.07)]">
                <div className="flex h-full items-stretch justify-between gap-[0.34vw]">
                  <div className="min-w-0 flex-1">
                    ...
                    <p className="mt-[0.08vw] line-clamp-2 text-[0.96vw] font-black leading-[1.05] text-[#171717]">{getKhmerName(product)}</p>
                    {getKhmerDescription(product) ? (
                      <p className="mt-[0.1vw] line-clamp-2 text-[0.46vw] leading-[1.14] text-slate-500">{getKhmerDescription(product)}</p>
                    ) : null}
                  ...
                  <div className="h-full min-h-[4.2vw] w-[4.2vw] shrink-0 overflow-hidden rounded-[0.72vw] bg-white">
                    <img src={imageUrl(product.image)} alt={getKhmerName(product)} className="h-full w-full object-cover" />
                  </div>
                </div>
              </div>
            ))
        ...
        <div className="relative h-full overflow-hidden rounded-[1.08vw] bg-[radial-gradient(circle_at_top,rgba(255,255,255,0.92),rgba(232,230,239,0.95))]">
          <img src={imageUrl(featured.image)} alt={getKhmerName(featured)} className="h-full w-full object-cover" />
          <div className="absolute inset-x-0 bottom-0 bg-[linear-gradient(180deg,rgba(255,255,255,0)_0%,rgba(20,20,20,0.72)_46%,rgba(20,20,20,0.94)_100%)] p-[0.58vw] text-white">
            <p className="text-[0.98vw] font-black leading-none">{getKhmerName(featured)}</p>
            <p className="mt-[0.18vw] max-w-[88%] text-[0.54vw] text-white/80">
              {getKhmerDescription(featured) || "Featured category item for display menu highlight"}
            </p>
            <div className="mt-[0.28vw]">
              <div className="space-y-0.5">
      ...
      {products.slice(0, 4).map((product) => (
        ...
        >
          <div className={`flex items-center justify-center overflow-hidden rounded-[0.72vw] bg-white ${dense ? "h-[4.2vw]" : "h-full min-h-[4.6vw]"}`}>
            <img src={imageUrl(product.image)} alt={getKhmerName(product)} className={`h-full w-full ${dense ? "object-contain p-[0.08vw]" : "object-cover"}`} />
          </div>
          <div className="min-w-0 self-center">
            <p className={`line-clamp-2 ${dense ? "text-[0.84vw]" : "text-[0.92vw]"} font-extrabold leading-[1.03] text-[#171717]`}>{getKhmerName(product)}</p>
      ...
      {products.slice(0, 8).map((product) => (
        <div key={product.id} className="grid min-w-0 grid-cols-[26%_1fr] items-center gap-[0.2vw] rounded-[0.8vw] bg-white/82 p-[0.18vw] shadow-[0_8px_18px_rgba(15,23,42,0.06)]">
          ...
          <div className="min-w-0">
            <p className="line-clamp-2 text-[0.84vw] font-extrabold leading-[1.03] text-[#171717]">{getKhmerName(product)}</p>
...
const MenuCardPosterPage = () => {
  ...
  const drinksSection = groupedCategories.find((entry) => (entry.key || "").toLowerCase() === "drinks");
  const mainSections = useMemo(
    () => groupedCategories.filter((entry) => normalizeCategoryName(entry.key) !== "drinks"),
    [groupedCategories]
  );
  ...
  const arrangedSections = useMemo(() => {
    ...
    const takeByMatcher = (matcher) => {
      const index = sections.findIndex((section) => matcher(normalizeCategoryName(section.key)));
      if (index === -1) {
        return null;
      }
